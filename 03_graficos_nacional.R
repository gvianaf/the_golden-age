...
# ---- gráfico de evolução dos indicadores da UnB - NACIONAL
...
graf_ind_nac <- dados_graf_ind %>% 
  ggplot(aes(x = year, y = valor, color = sigla)) +
  geom_line() +
  geom_point(size = 0.75, shape = 21) +
  facet_grid(cols = vars(indicador)) +
  scale_x_continuous(breaks = c(18, 19, 20)) +
  scale_color_manual(values = cores, guide = F) +
  labs(title = "Evolução dos indicadores das Universidades Brasileiras no Ranking THE <span style='color:#FFD700'>Golden Age</span>",
       subtitle = "A <span style='color:#0F4C81'>Universidade de Brasília</span> é a melhor em internacionalização dentre as 20 IES classificadas, além de ter<br>melhorado em quatro dos cinco indicadores, em especial nas citações, que correspondem a 30% da nota",
...
graf_ind_nac
ggsave("the-ga-ind-nacional.pdf", width = 8, height = 3, device = cairo_pdf)
pdftools::pdf_convert("the-ga-ind-nacional.pdf", format = "png", dpi = 350)

# ---- gráfico de evolução das posições - NACIONAL
...
graf_pos_nac <- dados_br %>% 
  ggplot(aes(x = year, y = country_rank, color = sigla)) +
  geom_point(size = 4) +
  ...
  geom_text(data = dados_br %>% filter(year == max(year)), family = "fira",
            aes(x = year + 0.5, label = glue::glue("{country_rank}ª"), size = 5, hjust = 0)) +
  scale_y_reverse(breaks = c(seq(1, 18))) +
  scale_x_continuous(limits = c(2017.6, 2020.5),
                     breaks = c(2018, 2019, 2020)) +
  scale_color_manual(values = cores, guide = F) +
  labs(title = "Evolução da posição das Universidades Brasileiras no Ranking THE <span style='color:#FFD700'>Golden Age</span>",
       subtitle = "Dentre as IES classificadas e que têm mais de 50 e menos de 80 anos desde sua criação,<br>a <span style='color:#0F4C81'>Universidade de Brasília</span> avançou para a 3ª posição nesta última edição do ranking",
       x = "Ano de divulgação do ranking",
       y = "", 
...
graf_pos_nac
ggsave("the-ga-nacional.pdf", width = 8, height = 5, device = cairo_pdf)
pdftools::pdf_convert("the-ga-nacional.pdf", format = "png", dpi = 350)

# ---- gráfico conjunto NACIONAL

graf_ind_nac / graf_pos_nac + plot_layout(heights = c(1, 3))
ggsave("the-ga-nacional_conjunto.pdf", width = 8, height = 8, device = cairo_pdf)
pdftools::pdf_convert("the-ga-nacional_conjunto.pdf", format = "png", dpi = 350)
